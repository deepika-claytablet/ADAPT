...
public class PTR_complex extends panToRelations{
    ...
    public String abc(MultiLevelPan mlp ,Pan pi){     
            Multimap<Pan, Pan> tree = mlp.getPancomplex(); 
            ...
            for (Pan i : children){
                ...
                if(!tree.get(i).isEmpty()) {
                    count =1;
                    output = output + "\n" + "alter table " + pi.getName() + " add "+ addColumn(i.getName()+"_SK")+  " ;";
                    output = output + "\n" + "alter table " + pi.getName() + " add foreign key ("+ i.getName() +  "_SK) references "+ 
